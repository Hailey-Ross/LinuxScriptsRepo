#!/bin/sh
#Warns users about an impending (planned) restart via Cron Job.
#Requires screen, otherwise comment out everything but the restart portion :shrugs:
screen -S YourScreenHere -p 0 -X stuff "say Preparing for Server Restart in 1 hour$(printf \\r)"
...
sleep 1
screen -S YourScreenHere -p 0 -X stuff "kick @a Server Restarting, please wait about 2 minutes before reconnecting.$(printf \\r)"
sleep 1
screen -S YourScreenHere -p 0 -X stuff "save-all$(printf \\r)"
sleep 3
